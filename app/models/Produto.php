...
INNER JOIN tbl_produtos AS p ON ip.id_produto = p.id_produto WHERE status_info_produtos = 'Ativo' AND link_produto = :link";
    ...
    public function addFotoProduto($id_produto, $arquivo, $nome_produto)
{
    // Verifique se o valor de link_produto (foto_produto) está vazio
    if (empty($arquivo)) {
        throw new Exception('O arquivo não pode ser vazio.');
    }

    // Certifique-se de que o id_produto esteja sendo passado corretamente
    if (empty($id_produto)) {
        throw new Exception('O ID do produto não pode ser vazio.');
    }

    // Inserção no banco de dados associando a foto ao produto
    $sql = "INSERT INTO tbl_produtos (foto_produto, alt_foto_produto, status_pedido, id_produto) 
            VALUES (:foto_produto, :alt_foto_produto, :status_pedido, :id_produto)";

    $stmt = $this->db->prepare($sql);
    $stmt->bindValue(':foto_produto', $arquivo);
    $stmt->bindValue(':alt_foto_produto', $nome_produto);
    $stmt->bindValue(':status_pedido', 'Ativo');
    $stmt->bindValue(':id_produto', $id_produto); // Vincule o ID do produto
    ...
    public function addproduto($dados)
    {
        ...
        $sql = "INSERT INTO tbl_produtos(nome_produto , preco_produto, status_pedido , id_categoria , alt_foto_produto, link_produto) 
VALUES(:nome_produto , :preco_produto, :status_pedido, :id_categoria ,:alt_foto_produto, :link_produto )";
        $stmt = $this->db->prepare($sql);
        ...
        $stmt->bindValue(':status_pedido', $dados['status_pedido']);
